...
try:
    ...
    st.error("Erro na conexão com Supabase. Verifique suas Secrets.")
...
def enviar_whatsapp(mensagem, telefone):
    try:
        ...
        st.warning(f"WhatsApp não enviado: {e}")
        return False

def atualizar_token_strava(refresh_token, strava_id):
    if not refresh_token:
        st.error("Refresh Token não encontrado. Reconecte ao Strava.")
        return None
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            for atv in atividades:
                ...
                # Upsert com tratamento de erro individual
                try:
                    supabase.table("atividades_fisicas").upsert(payload).execute()
                except Exception as e_db:
                    st.write(f"Aviso: Atividade de {atv['start_date_local']} já existe ou erro no banco.")
            ...
            st.error(f"Erro Strava: {res.status_code} - {res.text}")
            return False
    except Exception as e:
    ...
    with col2:
        ...
        with t2:
            ...
            if st.button("Cadastrar", use_container_width=True):
                supabase.table("usuarios_app").insert({"nome": n_c, "email": e_c, "senha": hash_senha(s_c), "telefone": t_c, "is_admin": False}).execute()
...
if res_strava.data:
    ...
    if st.sidebar.button("🔄 Sincronizar Agora", use_container_width=True):
        # Aqui passamos os dados garantindo que refresh_token não quebre o código
        rt = d.get('refresh_token')
        if sincronizar_dados(d['strava_id'], d['access_token'], rt, sel, st.session_state.user_info['telefone']):
