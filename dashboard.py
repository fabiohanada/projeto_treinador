...
# ==========================================
# VERSÃO: v5.7 (INTEGRAL v5.6 + STRAVA COMPLIANCE)
# ==========================================
...
def notificar_pagamento_admin(aluno_nome, aluno_email):
    try:
        ...
        if not check.data:
            supabase.table("alertas_admin").insert({"email_aluno": email, "mensagem": f"Novo pagamento detectado {aluno_nome.upper()}, por favor conferir na sua conta bancaria.", "lida": False}).execute()
                ...
                with st.expander("📄 Ver Termos de Uso e LGPD"):
                    st.write("""
                        **Termos de Uso e Privacidade - Fábio Assessoria**
                        ...
                        3. **Segurança:** Dados armazenados de forma segura e não compartilhados.
                    """)
...
with st.sidebar:
    ...
    if not eh_admin:
        ...
        # Ajuste de texto para conformidade Strava
...
# --- RODAPÉ OBRIGATÓRIO STRAVA ---
st.markdown("---")
cf1, cf2 = st.columns([8, 2])
with cf2:
    st.image("https://files.strava.com/api/v3/assets/api_logo_pwrdBy_strava_horiz_light.png")
