...
# ==========================================================
# VERSÃO: v7.1 (CORREÇÃO DE AVISOS STREAMLIT 2026)
# ==========================================================

st.set_page_config(page_title="Fábio Assessoria v7.1", layout="wide", page_icon="🏃‍♂️")
...
try:
    ...
    st.error("Erro crítico nas configurações (Secrets). Verifique o arquivo .streamlit/secrets.toml")
    st.stop()
    ...
    with c2:
        ...
        with tab_cadastro:
            with st.form("cad_form"):
                ...
                n_tel = st.text_input("Celular / WhatsApp")
                ...
                # CORREÇÃO AQUI: width="stretch" trocado por type="primary" (padrão novo) ou removido se der erro
    ...
    for aluno in alunos.data:
        with st.container(border=True):
            ...
            with c3:
                st.write("") 
                # CORREÇÃO AQUI: width="stretch"
                if st.button("💾 Salvar Data", key=f"s_{aluno['id']}", width="stretch"):
                ...
                if aluno['status_pagamento']:
                    # CORREÇÃO AQUI: width="stretch"
                    if st.button("🚫 Bloquear", key=f"b_{aluno['id']}", width="stretch"):
                        supabase.table("usuarios_app").update({"status_pagamento": False}).eq("id", aluno['id']).execute()
                        st.rerun()
                else:
                    # CORREÇÃO AQUI: width="stretch"
                    if st.button("✅ Ativar", key=f"a_{aluno['id']}", width="stretch"):
...
# --- 8. TELA DO ALUNO (DASHBOARD) ---
    ...
    if not df.empty:
        c1, c2 = st.columns(2)
        # Nota: Gráficos ainda podem usar use_container_width, pois o erro era específico dos botões
